...
| DDS-SHL-003 | Each concern-reference detail page has named sequence navigation with previous and next anchors using `rel="prev"` and `rel="next"`; boundary links correctly connect the index and final inventory back into the sequence. | Link/rel inventory for the page and, when reviewing the suite, a complete sequence traversal. |
